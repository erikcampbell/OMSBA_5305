...
summary(arma212_train) # RMSE = 1.330678, sigma^2 estimated as 1.78
# Coefficients:
#          ar1     ar2      ma1      ma2
...
# FORECAST MA2 - DYNLM
```{r}
fcast_ma2<-numeric(21) 
model_ma2 <-dynlm(working_hours_ts ~ L(residuals(working_hours_ts,-1)) + 
                    L(residuals(working_hours_ts,-2)),
                    start=c(2006,3), end=c(2021,6))

# Coefficients:
#               (Intercept)  L(working_hours_diff, -1)  L(working_hours_diff, -2)  
#                   0.06618                   -0.02778                   -0.16253 

for (i in 1:test_diff){ #start a for loop
  fcast_ma2[i]<-coef(model_ma2)[1]+coef(model_ma2)[2]*working_hours_diff[182+i]+coef(model2)[3]*working_hours_diff[181+i] # forecasted values at the end of each iteration
} 
...
model3 <- dynlm(working_hours_diff ~ L(working_hours_diff, -1) + L(working_hours_diff, -2) + L(residuals(model3), -1) + L(residuals(model3), -2))   # we'll have to look into.
