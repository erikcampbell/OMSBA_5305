...
TO DO:
1. OLS-weighted optimal forecast - emulate table 9.8 on text pp.245, already in slide deck in google drive

2. Regression Interpretation for 3 models to check the box.

3. Graphs - Haven't done anything for Combination Forecasts.
  a. Haven't done anything for Combination Forecasts.
  
4. Chris' narrative report checklist review

5. Update Slide deck with items list above.  Erik updated nearly everything.  Needs closers that want to win championships.

# Evaluating Heteroscedasticity
plot(arma22_forecast, resid(arma22fcast$residuals), xlab='Fitted Values', ylab='Residuals')
abline(0,0)
...
all_models <- cbind(test, arma22_forecast,ar2_forecast, ma2_forecast, f4,
                    ew_comb_fcast, imse_comb_fcast) # NOTE MISSING OLS WEIGHT
...
summary_table <- datatable(cbind(test, arma22_forecast, arma22error, arma22_loss, ar2_forecast,
                         ...
                         imse_comb_fcast, imse_error, imse_loss), # NOTE MISSING OLS WEIGHT
                         filter = "top", 
                          options = list(pageLength = nrow(test),autoWidth = TRUE),
...
## Qudratic Loss Data Tables
```{r}
model_losses <- datatable(cbind(arma22_loss, ar2_loss, ma2_loss, 
                                            f4loss,ew_loss,imse_loss),
                          caption = "Model Loss Data Table") 

# Mean Loss for Each Model Data Table
arma22_loss_mean <- mean(arma22_loss)
ar2_loss_mean <- mean(ar2_loss)
ma2_loss_mean <- mean(ma2_loss)
f4loss_mean <- mean(f4loss)
ew_loss_mean <- mean(ew_loss)
imse_loss_mean <- mean(imse_loss)

models_mean_loss <- datatable(round(cbind(arma22_loss_mean, ar2_loss_mean, ma2_loss_mean, 
                                    f4loss_mean, ew_loss_mean, imse_loss_mean), digits = 5),
                              caption = "All Models Mean Loss Out Of Sample")
models_mean_loss
```
