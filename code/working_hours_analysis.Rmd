...
We see a gradual ACF decay which suggests the data has a moving average component. Which when then looking at the PACF we can see that there is one sharp spike at the start. These two things suggest an MA of order 1 and 2 and AR(1) are worth exploring in our modeling process.
...
The COVID spike remains and will need to be addressed. With ACF and PACF spikes at the second lag suggests that a combined ARMA model of order 2 may be appropriate to consider. --*EC has question
...
```{r}
df_check2 <- ur.df(working_hours_diff, lags = 5)
summary(df_check) # t-Stat  =  -0.8495,   f-stat = 0.7773
                  # critical value: 
                        #tau2 -3.46 -2.88 -2.57 p-value = .3966 # super-high p-value
```
...
remove me?, I was following professor videos?(EC)
```{r}
...
# Coefficients:
                                 #  Estimate Std. Error t value Pr(>|t|)
# stats::lag(working_hours_diff, -1) -0.01800    0.07033  -0.256    0.798
# Adjusted R-squared:  -0.004625
# F-statistic: 0.06551 on 1 and 202 DF,  p-value: 0.7982
