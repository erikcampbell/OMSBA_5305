...
house_prices = read_excel("../data/HousePrices_CA.xls")
...
ggarrange(plot_work_hours1_density, plot_work_hours_diff_density, nrow =1) 

```
The density of index values plotted raw is not very symmetrical, as a result there are are concerns about this data being stationary and if transformations (i.e. taking first difference of index values and second index) are warranted. Let's start with the basics and explore for trends.
...
# Combine Raw Data & First Difference into One Plot
...
# Fit model ARMA(2,0,2) https://stats.stackexchange.com/questions/32634/difference-time-series-before-arima-or-within-arima - This Model is TOP 3 - Focus efforts here.
```{r, echo=FALSE}
...
# Fit model ARMA(2,1,0) This Model is TOP 3 - Focus efforts here.
```{r, echo = FALSE}
...
QTest<-numeric(3) 
for (i in 1:3){ 
  qt <- Box.test(model_ar2$residuals, lag=i, type="Ljung") 
  QTest[i]=qt$statistic
} 
...
The AIC is 685.56, both phi parameters are < 1, our adjusted r-squared is 0.9614188 and RMSE is 1.2769. Our Q-test using Ljung was 0.24938500 and p-value looked good at 0.9692 with three lags. The AIC was lower than ARMA 2,1,2 and adjusted r-squared was marginally better.  For pure AR models, the R-squared can be interpreted as usual. This model explains 96.14188% of the variation of the change in index of hours worked relating to base 2007 of 100.
...
# Fit Model MA(2) ARMA(0,1,2) This Model is TOP 3 - Focus efforts here.
```{r, echo=FALSE}
...
L(e) = ae^2 or Quadratic loss function.  h= 1.  Optimal forecast = conditional mean

```{r, echo = FALSE}
es_work <- window(working_hours_diff,start=c(2006,4),end=c(2021,6)) # difference 
length(es_work)# 183 observations
ps_work <- window(working_hours_diff, start=c(2021,7),end=c(2023,3)) # difference
length(ps_work) #21

######################### Base dataset TEST = not USING working_hours_diff   ###########################################
es_raw <- window(working_hours_ts,start=c(2006,4),end=c(2021,6)) #base data
length(es_raw) # 183 observations )
ps_raw <- window(working_hours_diff, start=c(2021,7),end=c(2023,3)) # note only 1 period
length(ps_raw) #21

model_arma2112_raw <- arima(es_raw, order = c(2,1,2))
summary(model_arma2112_raw) # AIC = 632.42  sigma^2 = 1.789 RMSE = 1.333841 MAE = .4737214
fcast1_raw <-numeric(21)
fcast1_raw <- forecast(model_arma2112_raw, h= 21) # Point Forecast    Lo 80    Hi 80    Lo 95    Hi 95   # note: predication sample forecast
                                           #Jul 2021 107.9125      106.1984  109.6265 105.2911 110.5339

e1_raw <- ps_raw - fcast1_raw$mean
MSE1_raw <-mean(e1_raw^2)


# Fixed Scheme Forecasting Environment

fcast1 <- numeric(21) #generate a vector of 21 zeros
g <- ts(working_hours$hours_worked_index, frequency = 12, start = c(2006, 3))
model <- dynlm(g ~ stats::lag(working_hours_ts,-1)+ stats::lag(working_hours_ts,-2),
             start=c(2006,3), end=c(2021,6)) #fit AR(2), last 21 in t.s. stops
for (i in 1:21){ #start a for loop
  fcast1[i]<-coef(g)[1]+coef(g)[2]*g[182+i] #fill in forecasted values at the end of each iteration
} #end the loop.
...
# ARMA(2,1,2) - Using as top 3
model_arma2112_estimate <- arima(es_work, order = c(2,1,2))
summary(model_arma2112_estimate) # AIC 638.21. sigma^2 = 1.789  RMSE = 1.334011, MAE = .4598895
